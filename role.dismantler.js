...
var roleDismantler={
    ...
    run: function(creep) {
        ...
        //     return -2;
        // }
        var targets = _.map(Memory.dismantleList,Game.getObjectById);
        var target=_.filter(targets,object=>object&&object.room.name==creep.room.name)[0];
        if(!target||target.room.name!=creep.room.name) return -1;
